...
//    List<String> findAllUserIds(); // 전체 사용자 ID 조회
	...
	/**
	 * 상품명(title) → 사용자 매칭 DAO impl이 없는 이유는 MyBatis의 mapper가 실제 실행 부분을 대신하기 때문
	 * 
	 ...
	 *
	 *         설명: - 새로운 상품이 크롤링되어 들어왔을 때 - 상품명(title)과 유저가 등록한 키워드를 비교하여 - 알림을 받아야
	 *         할 대상 사용자(userId)들을 추출하는 용도로 사용
	 */
